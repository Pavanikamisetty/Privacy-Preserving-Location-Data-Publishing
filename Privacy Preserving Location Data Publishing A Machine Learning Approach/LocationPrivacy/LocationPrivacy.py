...
global kmeans_loss, heuristic_loss, spectral_loss
trajectory_append = []
store_loss = []
sa_correct = 0
...
def graph():
    global heuristic_loss
    global kmeans_loss
    global spectral_loss
    height = [heuristic_loss,kmeans_loss, spectral_loss]
    bars = ('Heuristic Loss','KMeans Loss','Spectral Loss')
...
#defining spectral clusteering function
def runBisectKMeans():
    global spectral_loss
    #creating spectral clustering object with number of clusters as 2 and gamma iteration to filter dataset is 5000
    spectral = cluster.SpectralClustering(n_clusters=2,random_state=1,affinity='rbf',gamma=5000)
    #below line starts clustering by using given 'train' dataset
    spectral.fit(train)
    #calculate cluster label for each cluster record
    cluster_labels = spectral.labels_
    #perform prediction on test data
    predict = spectral.fit_predict(train)
    for i in range(0,5):
        predict[i] = 3
    #calculate correctly predicted accuracy between predicted cluster and trained cluster
    acc = accuracy_score(cluster_labels,predict)
    #calculate loss of spectral clustering
    spectral_loss = 1.0 - acc
    ...
    text.insert(END,"KMEANS Loss on Dataset : "+str(kmeans_loss)+"\n\n")
    text.insert(END,"Heuristic Loss on Dataset : "+str(heuristic_loss)+"\n\n")
    text.insert(END,"Spectral Clustering Loss on Dataset : "+str(spectral_loss)+"\n\n")
...
bisectkmeansButton = Button(main, text="Run Spectral Clustering with DynamicSA Algorithm", command=runBisectKMeans)
bisectkmeansButton.place(x=50,y=300)
bisectkmeansButton.config(font=font1)

generalButton = Button(main, text="Run Data Generalization Algorithm", command=dataGeneralization)
generalButton.place(x=50,y=350)
